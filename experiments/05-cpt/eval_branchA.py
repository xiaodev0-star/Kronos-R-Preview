"""Evaluate Branch A checkpoints under the 400-window protocol.

Builds a trajectory trial dir for a Branch A checkpoint (or its per-epoch
snapshots) and runs the formal 400-window evaluation, then compares against the
CPT baseline (exp04b_best_ep100.pt, balance 0.576 / JSD 0.319 / collapse 42.2%).

Usage:
    python experiments/05-cpt/eval_branchA.py \
        --ckpt checkpoints/branchA_dm010.pt \
        --tag branchA_dm010
"""
...
def build_index(ckpt: Path, tag: str, output_dir: Path, epochs: list[int]) -> Path:
    """Create a model_checkpoints.json for a Branch A run (or single ckpt)."""
...
def main() -> int:
    ...
    trial_dir = output_root / f"branchA_{args.tag}"
