"""Evaluate Branch B checkpoints under the 400-window protocol.

Builds a trajectory trial dir for a Branch B checkpoint (or its per-epoch
...
Usage:
    python experiments/05-cpt/eval_branchB.py \
        --ckpt checkpoints/branchB_ep6.pt \
        --tag branchB_ep6 \
        --epochs 1,2,3,4,5,6
"""
...
def build_index(ckpt: Path, tag: str, output_dir: Path, epochs: list[int]) -> Path:
    """Create a model_checkpoints.json for a Branch B run (or single ckpt)."""
...
def main() -> int:
    ...
    trial_dir = output_root / f"branchB_{args.tag}"
