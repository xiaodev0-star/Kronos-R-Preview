...
def _pad_batch(sequences, batch_size):
    ...
    for i in range(0, len(sequences), batch_size):
        ...
        # va_values [B, max_len, 2]
        p_va = torch.zeros(B, max_len, 2, dtype=torch.float32)
        # regression targets [B, max_len] (heteroscedastic head); -999 = ignore
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for bi, batch in enumerate(pbar):
            ...
            with torch.amp.autocast("cuda", dtype=amp_dtype):
                if use_heteroscedastic:
                    logits, _, _, het_loss = model(
                        inp, tid, pos, mask, va_values=va, reg_targets=reg_tgt)
                else:
                    logits, _ = model(inp, tid, pos, mask, va_values=va)
                    het_loss = None
